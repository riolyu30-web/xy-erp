...
def {{ tool.function_name }}({{ tool.args_str }}) -> str:  # 定义工具函数
    ...
    # 调用通用工具方法获取并过滤数据
    return fetch_data("{{ tool.alias_name }}", function_name, data, access_token, filtered_fields, meaning_list)  # 返回数据  
{% endfor %}
"""
...
def {{ tool.function_name }}_test(access_token: str):  # 定义测试函数
    ...
    # 调用通用工具方法获取并过滤数据
    fetch_data("{{ tool.alias_name }}", function_name, data, access_token, filtered_fields, meaning_list, debug_mode=True)
{% endfor %}
...
def generate_service_script(json_path: str):  # 定义生成服务脚本的函数
    ...
    for service_config in mcp_list:
        ...
        for tool in tools_data:  # 遍历每一个工具定义
            ...
            to_field = None
            for f in filters:
                if "from" in f.get("field", "").lower() or "start" in f.get("field", "").lower():
                    from_field = f.get("field")
                if "to" in f.get("field", "").lower() or "end" in f.get("field", "").lower():
                    to_field = f.get("field")
